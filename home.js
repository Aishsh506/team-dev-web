...
$(function () {
	...
	$('#delete-teacher-Btn').click(async function(){
		if(TeacherData.has($('#delete-teacher-DataList').val())){
			console.log("Преподователь существует"+TeacherData.get($('#delete-teacher-DataList').val()))
...
//
	...
	roleSelect.on({
        change: function() {
            ...
            else if($(this).val()!="0" || !bindTeacherInputForm.hasClass("d-none"))
            {
    ...
    GroupDropdown.bind('hide.bs.dropdown',event=>{groupListDropDownInput.val('')})
    TeacherDropdown.bind('hide.bs.dropdown',event=>{teacherListDropDownInput.val('')})
    AuditoriumDropdown.bind('hide.bs.dropdown',event=>{buildingListDropDownInput.val('');roomListDropDownInput.val('');roomListDropDownInput.prop('disabled', true);})
...
function GroupShcedule() {
	if(GroupData.has(groupListDropDownInput.val())){
		...
		window.location.href = "/schedule/?method=byGroup&id1="+GroupData.get(groupListDropDownInput.val())
	}
	else{
...
function TeacherShcedule() {
	...
	if(TeacherData.has(teacherListDropDownInput.val())){
		...
		window.location.href = "/schedule/?method=byTeacher&id1="+TeacherData.get(teacherListDropDownInput.val())
	}
	else{
...
function AuditoriumShcedule() {
	if(!BuildingData.has(buildingListDropDownInput.val())){
		...
		window.location.href = "/schedule/?method=byRoom&id2="+BuildingData.get(buildingListDropDownInput.val()) +'&id1=' + RoomData.get(roomListDropDownInput.val())
	}
}//TODO
